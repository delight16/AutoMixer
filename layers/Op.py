...
def create_op(op_name, d_model, d_ff):
    ...
    name2op = {
        'Zero': lambda: Zero(), 
        'Identity': lambda: Identity(), # 观察权重change
        ...
        #
        # change增加平均池化，最大池化，卷积池化等操作 增加卷积块DARTS论文里的  
        'AvgPool': lambda: AvgPool(),
        'MaxPool': lambda: MaxPool(),
        # 'ConvPool': lambda: ConvPool(),

        # 'avg_pool_3x3': lambda: PoolBN('avg', d_model, 3, stride, 1, affine=False),
        # 'max_pool_3x3': lambda: PoolBN('max', d_model, 3, stride, 1, affine=False),
        'sep_conv_3x3': lambda: SepConv(d_model, d_ff, 3, stride, 1, affine=False),
        'sep_conv_5x5': lambda: SepConv(d_model, d_ff, 5, stride, 2, affine=False),
        'sep_conv_7x7': lambda: SepConv(d_model, d_ff, 7, stride, 3, affine=False),
        # 'dil_conv_3x3': lambda stride: DilConv(d_model, d_ff, 3, stride, 2, 2, affine=False), # 5x5
        # 'dil_conv_5x5': lambda stride: DilConv(d_model, d_ff, 5, stride, 4, 2, affine=False), # 9x9
        # 'conv_7x1_1x7': lambda stride: FacConv(d_model, d_ff, 7, stride, 3, affine=False)
...
class MixedOp(BasicOp):
    ...
    def forward(self, inputs):
        # if self.mode == Mode.TWO_PATHS
            #离散化后没有softmax change
        # probs = F.softmax(self._candidate_alphas[self._sample_idx], dim=0) #离散化后没有softmax
        # output = 0
        # for i, idx in enumerate(self._sample_idx):
        #     output += probs[i] * self._candidate_ops[idx](inputs)
        # return output

        if self._mode == Mode.TWO_PATHS:
            # 直接选择最可能的操作，而不是进行 softmax
            output = self._candidate_ops[self._sample_idx[0]](inputs)
        else:
...
class SepConv(nn.Module):
    ...
    def __init__(self, C_in, C_out, kernel_size, stride, padding, affine=True):
        super().__init__()
        self.net = nn.Sequential(
            DilConv(C_in, C_out, kernel_size, stride, padding, dilation=1, affine=affine),
            DilConv(C_out, C_in, kernel_size, 1, padding, dilation=1, affine=affine)
        )
