...
from layers.Op import candidate_op_profiles, Cell
...
class Model(nn.Module):  #继承自nn.Module类，用于定义一个神经网络模型
    ...
    def set_arch_parameters(self, arch_parameters): 
        assert torch.is_tensor(arch_parameters)
        arch_parameters_list = torch.chunk(arch_parameters, self.configs.e_layers)  #等分
        for l in range(len(self.pdm_blocks)):
            self.pdm_blocks[l].set_arch_parameters(arch_parameters_list[l]) 
...
class PastDecomposableMixing(nn.Module):
    def __init__(self, configs):
        ...
        self.Nas_mix_cells = nn.ModuleList(
            [Cell(configs, configs.op_num, candidate_op_profiles) for _ in range(5)]
        )
        # self.Nas_mix_i = nn.ModuleList([Cell(configs, configs.op_num, candidate_op_profiles)
    ...
    def Nas(self, x_list, t = 0):
        ...
        for i in range(len(x_list)):
            ...
            if(len(x_list) == 4):
                out_x_list.append((self.Nas_mix_cells[0](scale_x_list[0])+self.Nas_mix_cells[1](scale_x_list[1])+self.Nas_mix_cells[2](scale_x_list[2])+self.Nas_mix_cells[3](scale_x_list[3])).permute(0, 2, 1))
            elif(len(x_list) == 3):
                out_x_list.append((self.Nas_mix_cells[0](scale_x_list[0])+self.Nas_mix_cells[1](scale_x_list[1])+self.Nas_mix_cells[2](scale_x_list[2])).permute(0, 2, 1))
            else:
                out_x_list.append((self.Nas_mix_cells[0](scale_x_list[0])+self.Nas_mix_cells[1](scale_x_list[1])).permute(0, 2, 1))

            # for i in range(len(x_list)):
            #     out_x_list += self.Nas_mix_cells[i](scale_x_list[i]).permute(0, 2, 1)  # 2. 堆叠 Cell 计算
        
            # out_x_list.append(stacked_output.permute(0, 2, 1))
            # out_x_list.append(stacked_output)
    ...
    def set_arch_parameters(self, arch_parameters):
        assert arch_parameters.size(0) == len(candidate_op_profiles) * self.configs.op_num
        self.Nas_mix.set_arch_parameters(arch_parameters)
