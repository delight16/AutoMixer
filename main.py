...
from m4run import Exp_Short_Term_Forecast
import time
import logging
...
parser = argparse.ArgumentParser(description='TimesNet')  #创建一个ArgumentParser对象，用于解析命令行参数。description参数是关于脚本功能的简短描述。
...
parser.add_argument('--model', type=str, required=True, default='Autoformer',
                    help='model name, options: [Autoformer, Transformer, TimesNet]') #模型名称 可选
...
parser.add_argument('--checkpoints', type=str, default='./check48/', help='location of model checkpoints') #模型检查点 快照信息
...
parser.add_argument('--dec_in', type=int, default=7, help='decoder input size') #解码器输入大小
...
parser.add_argument('--gpu', type=int, default=2, help='gpu') #选择的GPU编号
parser.add_argument('--use_multi_gpu', action='store_true', help='use multiple gpus', default=False) #是否使用多个GPU
parser.add_argument('--devices', type=str, default='0,1', help='device ids of multile gpus') #多个GPU的设备ID
...
parser.add_argument('--op_num', type=int, default=2)#候选操作层数
...
setting = 'alpha_{}_seed{}_batch{}_lr{}_epoch{}_in{}_out{}_dm{}_el{}_dl{}_df{}_op{}'.format(
